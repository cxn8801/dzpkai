...
class WebSocketClient:
    ...
    async def on_enter_room(self, data):
        ...
        self.seat_info[data.roomPlayer.seatNumber] = data.roomPlayer.uid
    ...
    def get_player_position(self, seat_dict, dealer_uid, player_uid):
        """
    ...
    async def choice_operation(self, opButtons):
        ...
        game_state['player_id'] = self.my_uid
        game_state['hero_hand'] = [create_card(self.my_hand_cards[0]), create_card(self.my_hand_cards[1])]
        board = []
        for c in self.table_cards:
            board.append(create_card(c))
        game_state['community'] = board
        if self.room_status == RoomStatus.RoomStatusPreFlop:
            game_state['street'] = 'preflop'
        if self.room_status == RoomStatus.RoomStatusFlop:
            game_state['street'] = 'flop'
        if self.room_status == RoomStatus.RoomStatusTurn:
